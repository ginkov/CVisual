...
def parse(text):
    ...
    for _line in text:
        ...
        m_line = re.match(r'(\d+)\s+bytes\s+used', _line, re.M | re.I)
        if m_line:
            m_used = int(m_line.group(1))
            continue

        m_line = re.match(r'(\d+)\s+bytes\s+free', _line, re.M | re.I)
        if m_line:
            m_free = int(m_line.group(1))

    results[0]['total'] = round((m_free + m_used)/1024.0, 2)
    results[0]['used'] = round(m_used/1024.0, 2)
    results[0]['free'] = round(m_free/1024.0, 2)
